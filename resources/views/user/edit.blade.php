        ...
        <div class="card-body">
            ...
            @else
                <form method="POST" action="{{ url('/user/' . $user->user_id) }}" class="form-horizontal" enctype="multipart/form-data">
                    @csrf
                    {!! method_field('PUT') !!} <!-- Tambahkan baris ini untuk menggunakan method PUT -->
                    ...
                    <div class="form-group row">
                        ...
                        <div class="col-11">
                            @if ($user->image)
                                <img src="{{ asset('storage/user/' . basename($user->image)) }}" class="img-thumbnail" alt="Profile Picture">
                            @else
                            ...
                            @error('image')
                                <small class="form-text text-danger">{{ $message }}</small>
                            @else
                                <small class="form-text text-muted">Abaikan (jangan diisi) jika tidak ingin mengganti foto profil user.</small>
                            @enderror
                        </div>
                    </div>
